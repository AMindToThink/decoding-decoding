...
2. **The certificate** — grid mixture = Vovk AA; Lemma 1 (already in tex, lines 283-307); MAP = grid MLE identity; level-set bar; the caveats up front: in-hindsight target (never the dial), widening on peaked logits, guarantee attaches to the mixture, **narrow bar = identifiability ≠ goodness of fit**. Mine `grid_certificate_walkthrough.html` for prose — it is the best existing exposition (experts framing, telescoping proof). **Mode-vs-mean correction (do NOT regress this):** do **not** import the walkthrough's §7 bimodal "posterior-mean trap" example (½ on β=0.1, ½ on β=10, valley at β≈5). It contradicts the convexity/level-set bar presented in this very section: Lₙ(β) convex ⟹ posterior ∝ e^{−Lₙ} is log-concave ⟹ **unimodal**, so a two-peak posterior with an interior valley is impossible (chord inequality: the midpoint's weight is ≥ either endpoint's). Justify reporting the mode via the **MAP = best-fit identity** (zero Jensen slack). If a mean-vs-mode remark is wanted, the only honest one is: the posterior is **unimodal-but-skewed**, so the grid posterior *mean* is a *mildly* biased estimate of the grid MLE — not a catastrophe. The legacy `discrete_grid_update_batched` docstring already uses exactly this "skewed → biased" framing; keep it, do not escalate to the bimodal story anywhere in the paper.
